...
public:
    ...
    template<core::StringLiteral Name> 
    inline xcb_atom_t resolveXCBAtom(XCBAtomToken<Name>& token) const {
        ...
        xcb_intern_atom_cookie_t cookie = xcb.pxcb_intern_atom(m_connection, 0, size, Name.value);
        xcb_intern_atom_reply_t* reply = xcb.pxcb_intern_atom_reply(m_connection, cookie, nullptr);
        token.token = reply ? reply->atom : 0;
        free(reply);
        return token.token;
    }
